...
object Option {
  ...
  def map2[A,B,C](a: Option[A], b: Option[B])(f: (A, B) => C): Option[C] = a match {
    case Some(as) => b match {
      case Some(bs) => Some(f(as, bs))
      case None => None
    }
    case None => None
  }
  ...
  // Exercise 4.5
  // Pro-tip if you have a map and then a sequence, you can generally rewrite it as a traverse.
  def traverse[A, B](as: List[A])(f: A => Option[B]): Option[List[B]] = {
    val listOfOptionB: List[Option[B]] = map(as)((a : A) => f(a))
